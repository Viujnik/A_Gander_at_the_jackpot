...
+ Дева - интересно проводит время с игроком за символичную оплату. Имеет цвет глаз и волос, при цвете волос блонд и зеленых/синих глазах - цена повышается на 5.
...
+ WhoreCollection - список для хранения дев казино.
...
+ whore_time - логика взаимодействия игрока и дев.
...
# Тесты
Для тестов используется pytest. Чтобы запустить тесты в консоли(будучи в корне проекта) напишите:
```bash
  pytest
```
Код для тестов находится в tests/. Разделены они по следующей логике:
+ confest.py - общие фикстуры для последующего использования их в тестах, чтобы не писать всё с нуля.
+ test_models.py - тестирование базовых атрибутов/методов классов персонажей и фишек.
+ test_collection.py - тесты для методов коллекций, хранящих баланс игроков, список гусей/игроков и фишек казино.
+ test_simulation_step.py - проверка работы функций, отвечающих за логику каждого шага симуляции.
+ test_casino.py - тестирование методов казино - регистрация гусей/игроков.

###### Интересные вещи:
+ название проекта - попытка использовать игру слов: gander - гусак(с англ.), но есть еще сленг - take a gander - смотреть на кого-то или на что-то, особенно быстро, непринужденно или небрежно. Поэтому перевод названия для такой: Гусиный взгляд на джекпот. Помните? У нас очень необычное понимание казино - можешь получить денюшку, но если проиграл всё до копейки - полиция заберёт вас(надеюсь не заспойлерил вам эту фичу).
+ pido_random_simulation.py - ...Когда я думал о псевдо-случайности бытия и не только, я написал слово pido - всё остальное промысел судьбы.
+ человечество не может доказать, что мы живем не в симуляции.
+ не играйте в казино. Но если это игра с супер-близкими друзьями и на дружеский интерес - можно. И помните, Гуси следят за тобой.
